...
static void	print_instructions(char **arr, int size1, int size2)
{
	...
	i = 0;
	j = 0;
	while (i < size1 && j < size2 && ROT == arr[0][i] && ROT == arr[1][j] && ++i && ++j)
		put_instruction(RR);
	while (i < size1 && j < size2 && RROT == arr[0][i] && RROT == arr[1][j] && ++i && ++j)
		put_instruction(RRR);
	while (i < size1\
...
static void	fix_position_a(t_head *head, t_stack *lc)
{
	...
	while (tmp->next && tmp != lc && ++position)
		tmp = tmp->next;
	while ((position <= a_size / 2) && position > 0 && rotate_a(head, 0) && stack_instructions(0, ROT))
		--position;
	while ((position > a_size / 2) && position < a_size && r_rotate_a(head, 0) && stack_instructions(0, RROT))
		++position;
}
...
static void	fix_position_b(t_head *head, t_stack *lc)
{
	...
	while ((position <= b_size / 2) && position > 0 && rotate_b(head, 0) && stack_instructions(1, ROT))
		--position;
	while ((position > b_size / 2) && position < b_size && r_rotate_b(head, 0) && stack_instructions(1, RROT))
		++position;
}
